 ...
 * on an attention. They are symmetric: terminating a clinical case requires
 * the practitioner to have left no SCHEDULED appointments dangling — neither
 * future (must be cancelled first) nor past (must be marked completed or
 * no-show). Used to preemptively disable the buttons with a precise tooltip
 * so the user doesn't see a 422.
 */
